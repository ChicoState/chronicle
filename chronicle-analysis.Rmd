...
ggplot(data=github_by_individual, aes(x=consistency, y=frequency, )) + geom_jitter(alpha=0.33, width = 0.02) + theme_minimal() + scale_size(range = c(1, 10)) 
```
...
summary(lm(data=team_outcomes, Cohesiveness~Median_Consistency))
```
...
summary(lm(data=team_outcomes, Conflict~Median_Artifacts+Median_Consistency))
```